  ...
  const addNewPhoneNumberHandler = (event) => {
    ...
    if (duppArr.length > 0) {
      if(window.confirm(`${newName} is already added to the phonebook, replace the old number with a new one?`)){
        ...
        const newPersonData = { id: personData.id, name: personData.name, number: newNumber };
        
        personService.editPerson(personData.id, newPersonData).then(newData => {
          setPersons(persons.map(person => person.id !== personData.id ? person : newData));
    ...
    personService.createPerson(newPerson).then(newlyCreatedPerson => {
      ...
      copiedPhoneBook = copiedPhoneBook.concat(newlyCreatedPerson);
