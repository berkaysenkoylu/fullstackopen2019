...
// Like the blog post with a given id
exports.likeBlogPost = (req, res, next) => {

    Blog.findById({ _id: req.params.id }).then(fetchedBlog => {

        const copiedBlogPost = {...fetchedBlog._doc};
        copiedBlogPost.likes++;

        Blog.updateOne({ _id: req.params.id }, copiedBlogPost).then(result => {
            
            if (result.n > 0) {
                return res.status(200).json({
                    message: "Liked the blog post",
                    data: copiedBlogPost
                });
            }
            else {
                res.status(500).json({
                    message: "Failed to like the blog post"
                });
            }
        })
    }).catch(error => {
        return res.status(404).json({
            message: "Couln't find the post with the given id",
            error: error
        });
    });
}